...
SELECT s.`shopname`
FROM `shops` s, `cats` c
WHERE s.id = c.shops_id AND c.`name` NOT IN('Murzik', 'Zuza');

SELECT s.`shopname`
FROM `shops` s
WHERE s.`shopname` NOT IN (SELECT s1.`shopname`
FROM `shops` s1
JOIN `cats` c
ON s.id = c.shops_id
...
FROM analysis a, `Orders` o
WHERE a.an_id = o.ord_an AND (o.ord_datetime BETWEEN '2020-02-05 00:00:00' AND '2020-02-12 23:59:59');
